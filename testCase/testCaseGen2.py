...
def get_sd_over_s(jobs):
    sd_sum = 0
    slice_num = 0
    for job in jobs:
        for task in job:
            print('task[0][0]', task[0][0])
            print('task[0][3]', task[0][3])
            sd_sum+=task[0][0]*task[0][3]
            slice_num+=task[0][3]
    result = sd_sum/slice_num
    return result
...
if __name__ == "__main__":
    ...
    #print(jobs)
    fintime =  flexible_jobshop(jobs, weight, slice_num, name)
    sd_over_s = get_sd_over_s(jobs)
    metric = fintime/sd_over_s
    print('sd_over_s', sd_over_s)
    print('metric', metric)
    print('result', fintime)